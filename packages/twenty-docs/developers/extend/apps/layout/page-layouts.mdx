...
- Existing manifests with an explicit `position` remain supported. For `VERTICAL_LIST`, the parent tab and widget array still determine `layoutMode` and `index`, while a nested `position.heightBehavior` is used as a legacy fallback. If both nested and top-level height behaviors are present, the top-level value takes precedence.
- `PageLayoutTabLayoutMode.CANVAS` is deprecated. During the compatibility window, Twenty converts a legacy Canvas manifest only when the tab has exactly one widget; empty and multi-widget Canvas tabs remain unchanged because there is no unambiguous widget to make viewport-filling.
