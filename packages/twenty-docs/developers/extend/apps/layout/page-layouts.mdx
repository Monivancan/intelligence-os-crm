...
- A widget `position` matches its tab layout mode. `VERTICAL_LIST` uses `{ layoutMode: PageLayoutTabLayoutMode.VERTICAL_LIST, index, heightBehavior? }`, while `GRID` uses `{ layoutMode: PageLayoutTabLayoutMode.GRID, row, column, rowSpan, columnSpan }`. Set `heightBehavior` to `PageLayoutWidgetVerticalListHeightBehavior.TAB_VIEWPORT` when this specific widget should fill the remaining tab viewport and own its scrolling; use `FIT_CONTENT` to override a built-in full-height default. Only one active `TAB_VIEWPORT` widget is allowed per tab, and it must come after fit-content widgets.
